...
def calculate_hysteresis_loop(Ms, Hc, a, k, C_max=-250, T_max=550, num_points=200):
    """
    Calculates the x and y coordinates for a hysteresis loop using a tanh model.

    Args:
        Ms (float): Saturation magnetization.
        Hc (float): Coercivity.
        a (float): Squareness parameter.
        k (float): Slope of the ends, related to Young's Modulus.
    ...
    Returns:
        tuple: A tuple containing two numpy arrays (x_loop, y_loop).
    """
    # Prevent division by zero if 'a' is zero
    if a == 0:
        a = 1e-9

    # Generate the magnetic field values (x-axis) for the two branches
    y_asc = np.linspace(C_max, T_max, num_points)  # Ascending branch
    y_desc = np.linspace(T_max, C_max, num_points) # Descending branch

    # Calculate the magnetization values (y-axis) for each branch
    x_asc = Ms * np.tanh((y_asc + Hc) / a) + k * y_asc  # Upper curve
    x_desc = Ms * np.tanh((y_desc - Hc) / a) + k * y_desc  # Lower curve
...
class HysteresisPlotter(QMainWindow):
    ...
    def __init__(self):
        ...
        self.plot_widget.setTitle("Hysteresis Loop Control")
        ...
        # --- Saturation (Ms) Slider ---
        ms_group = QHBoxLayout()
        ms_label = QLabel("Saturation (M_s):")
        ...
        # --- Coercivity (Hc) Slider ---
        hc_group = QHBoxLayout()
        hc_label = QLabel("Coercivity (H_c):")
